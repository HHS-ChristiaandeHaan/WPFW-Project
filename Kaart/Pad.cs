...
public class Pad : Tekenbaar
{
    ...
    public void TekenConsole(ConsoleTekener tekener)
    {
        for (int i = 0; i < (int)Lengte(); i++)
        {
            ...
            tekener.SchrijfOp(new Coordinaat((int)Math.Round(van.x + (naar.x - van.x) * factor), (int)Math.Round(van.y + (naar.y - van.y) * factor)), "#");
        }
        tekener.SchrijfOp(new Coordinaat((int)Math.Round(van.x + (naar.x - van.x) * .5), (int)Math.Round(van.y + (naar.y - van.y) * .5)), (1000 * Lengte()).metSuffixen());
    }
}